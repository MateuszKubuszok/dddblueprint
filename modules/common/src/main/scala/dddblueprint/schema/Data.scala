...
object Data {
  ...
  object Definition {
    ...
    @Semi(Eq, Show) sealed abstract class Record(override val ref: DefinitionRef) extends Definition(ref)
    object Record {

      @Semi(Eq, Show) final case class Tuple(override val ref: DefinitionRef, fields: FieldSet) extends Record(ref)

      @Semi(Eq, Show) final case class Entity(override val ref: DefinitionRef, fields: FieldSet) extends Record(ref)
      @Semi(Eq, Show) final case class Value(override val ref:  DefinitionRef, fields: FieldSet) extends Record(ref)
      @Semi(Eq, Show) final case class Event(override val ref:  DefinitionRef, fields: FieldSet) extends Record(ref)
    }
